...
int16_t mavlink_active_param_count = 0;
			...
			case MAV_CMD_PREFLIGHT_STORAGE:
				if (packet.param1 == MAV_PFS_CMD_WRITE_ALL)
				{
					if (packet.param2 == MAV_PFS_CMD_WRITE_ALL)
						save_parameters(STORAGE_FLAG_ALL, &preflight_storage_complete_callback);
					else
						save_parameters(STORAGE_FLAG_STORE_CALIB, &preflight_storage_complete_callback);
				}
				else if (packet.param1 == MAV_PFS_CMD_READ_ALL)
				{
					if (packet.param2 == MAV_PFS_CMD_READ_ALL)
						load_parameters(STORAGE_FLAG_ALL, &preflight_storage_complete_callback);
					else
						load_parameters(STORAGE_FLAG_STORE_CALIB, &preflight_storage_complete_callback);
				}
				else
					command_ack(packet.command, MAV_CMD_ACK_ERR_NOT_SUPPORTED);
				break;

//			case MAV_CMD_PREFLIGHT_STORAGE_ADVANCED2:
...
//		********************* END of WAYPOINT SECTION not converted to wire protocol 1.0 *****/
		...
		case MAVLINK_MSG_ID_PARAM_REQUEST_LIST:
		{
			...
			send_variables_counter = 0;
                        mavlink_active_param_count = get_active_param_count(false);
		...
		case MAVLINK_MSG_ID_PARAM_REQUEST_READ:
		{
			...
			if (packet.target_system != mavlink_system.sysid) break;
			if ((packet.param_index >= 0)&& (packet.param_index <= get_active_param_count(false)))
			{
...
//				send_text((uint8_t*) "failed target system check on parameter set \r\n");
			...
			else
			{
				...
				const char * key = (const char*) packet.param_id;
                                ...
                                uint16_t handle = get_param_handle(key);

                                if(handle != INVALID_PARAMETER_HANDLE)
                                {
                                    const struct param_info_s* pparam = get_param(handle);

                                    if(pparam->readOnly == false)
                                    {
                                        mavlink_param_union_t param;
...
//		}
	...
	{
		if (send_variables_counter < mavlink_active_param_count)
		{
                        const struct param_info_s* pparam = get_param(send_variables_counter);
                        mavlink_param_union_t param;

                        if(pparam != NULL)
                        {
                            param = param_scale_read(pparam);
                            mavlink_msg_param_value_send(MAVLINK_COMM_0, pparam->name,
                                param.param_float, param.type, mavlink_active_param_count, send_variables_counter);
                        }
	...
	{
// TODO		mavlink_parameter_parsers[mavlink_parameters_list[send_by_index].udb_param_type].send_param(send_by_index);

            const struct param_info_s* pparam = get_param(send_by_index);
            mavlink_param_union_t param;
            ...
            if(pparam != NULL)
            {
                param = param_scale_read(pparam);
                mavlink_msg_param_value_send(MAVLINK_COMM_0, pparam->name,
                    param.param_float, param.type, mavlink_active_param_count, send_by_index);
            }
